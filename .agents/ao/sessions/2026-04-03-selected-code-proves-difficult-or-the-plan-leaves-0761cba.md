...
- tilization vs capacity
- **Grove tier lifecycle tracking** — promotion/demotion rates between GPU→CPU→SSD, per-tier hit rates over time
- **Worker-level disaggregation balance** — prefill vs...
- till says "21 audit checks" but there are now 31. The CLAUDE.md also references "21 ISA-grounded audit checks" in the optimization section.

2. **Is the deep-research-agent workload validated against...
...
| mcp__RepoPrompt__git | 3 |
| mcp__RepoPrompt__read_file | 35 |
